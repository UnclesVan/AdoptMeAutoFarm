             --BEGIN STROLLER LOGIC----


-- This script is an updated version that runs the action in a continuous loop
-- without producing a 'nil' error.

-- Wrap the entire loop in task.spawn(). This runs the code on a separate thread,
-- so your main script will not be interrupted or stopped.
task.spawn(function()
	while true do
		-- Get the local player. The script will not proceed if the player is not found.
		local localPlayer = game:GetService("Players").LocalPlayer
		
		-- Get the Pets folder. This will wait until the folder is available.
		local petsFolder = workspace:WaitForChild("Pets")
		
		-- Wait until a Model is found inside the 'Pets' folder before continuing.
		-- This prevents the "nil" error by ensuring the pet model exists.
		local firstPetModel = petsFolder:FindFirstChildOfClass("Model")
		while not firstPetModel do
			task.wait(0.1) -- Wait a fraction of a second and try again.
			firstPetModel = petsFolder:FindFirstChildOfClass("Model")
		end
		
		-- Define the arguments for the InvokeServer call.
		-- All arguments are now guaranteed to exist.
		local args = {
			localPlayer,
			firstPetModel,
			-- The third argument, which is the stroller part.
			-- This will also wait for the parts to be available in the player's character.
			localPlayer.Character:WaitForChild("StrollerTool"):WaitForChild("ModelHandle"):WaitForChild("TouchToSits"):WaitForChild("TouchToSit")
		}

		-- Call the remote function on the server with the unpacked arguments.
		-- The check is no longer needed since we are waiting for all objects to exist.
...
print("✅Pet Stroller script Loaded Loading script...")
